...
namespace CLMS
{
    public class WMBP : LMSBase
    {
        ...
        public WMBP(Endian aByteOrder, Encoding aEncoding, bool createDefaultHeader = true) : base(aByteOrder, aEncoding, createDefaultHeader, FileType.WMBP) { }
        ...
        {
            var reader = CreateReadEnvironment(stream);
        ...
        private WLNG ReadWLNG(BinaryStream reader)
        {
            WLNG result = new();
            ...
            for (uint i = 0; i < languagesNum; i++)
            {
                ...
                byte cLanguageUnk0 = reader.Read1Byte();
                string cLanguageName = reader.ReadString(StringCoding.ByteCharCount, Encoding.ASCII);
        ...
        private WSYL ReadWSYL(BinaryStream reader, int numberOfLanguages)
        {
            WSYL result = new();
        ...
        private WFNT ReadWFNT(BinaryStream reader)
        {
            WFNT result = new();
